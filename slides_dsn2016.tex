...
\makeatother
\author{Jiguo Song, Gedare Bloom, Gabriel Parmer \\Presented by {\bf Teo Georgiev}}
%\author{Presented by {\bf Teodor Georgiev}}
...
\begin{frame}{Application-Level Fault Tolerance}
  \begin{columns}
    ...
    \begin{column}{0.35\textwidth}
      \vspace{-3em}
      \only<1>{\includegraphics[width=1\textwidth]{figures/app2}}
      \only<2>{\includegraphics[width=1\textwidth]{figures/app1}}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{System-Level Fault Tolerance}
  ...
  \onslide<3->{ \bigskip Recovery requires resources
    \begin{itemize}
    \item processing time...to recover scheduler
    \item memory...to recover memory manager
    \end{itemize}
  }
\end{frame}
...
\begin{frame}{\c3\ Implementation -- Writing Code Manually}
  ...
  \only<4>{\includegraphics[width=0.9\linewidth]{SuperGlueFigs/motivation/code5}}%
  \only<5>{\includegraphics[width=0.9\linewidth]{SuperGlueFigs/motivation/code6}}%
...
\begin{frame}{\sg\ -- IDL-based System-Level Fault Tolerance}
  \begin{columns}
    ...
    \begin{column}{0.6\textwidth}
      ...
      %  mechanisms
      \item a \blue{compiler} synthesizes recovery code from the
        system model
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
  ...
  \begin{columns}
    ...
    \begin{column}{1.1\textwidth}
      ...
      \begin{itemize}
      \item on-chip single-event upset (SEUs)
      \item fault is detected immediately ({\sl fail-stop})
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{\sg\ -- IDL-based System-Level Fault Tolerance}
  \begin{textblock}{110}(8,14)
    \only<1>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/overview_1}}%
    \only<2>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/overview_2}}%
    \only<3>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/overview_3}}%
  \end{textblock}
\end{frame}

\begin{frame}
  \frametitle<1-2>{Interface Description Model -- Descriptor and Resource}
  \begin{textblock}{110}(8,14)
    \only<1>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_fs_no_res}}%
    \only<2>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_fs_interface_funcs}}%    
    %\only<3>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_fs}}%
  \end{textblock}
  \begin{textblock}{52}(72,15)
    \begin{itemize}
      \only<1->{\item File descriptor (descriptor)}
    \end{itemize}
    \begin{itemize}
      \only<2->{\item File system provides interface functions}
      \begin{itemize}
        \only<2->{\item open/close/read/write...}
      \end{itemize}
    \end{itemize}
    \begin{itemize}
      \only<2->{\item Interface functions operate on the file
        descriptor}
    \end{itemize}
    %\begin{itemize}
     % \only<3>{\item Access the file (resource)}
    %\end{itemize}
  \end{textblock}
\end{frame}
...
%   \frametitle<1-2>{Interface Description Model -- Descriptor and Resource}
%   \begin{textblock}{110}(8,14)
%     \only<1>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_thread_no_res}}%
%     \only<2>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_thread_interface_funcs}}%    
%     %\only<3>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_thread}}%
%   \end{textblock}
%   \begin{textblock}{52}(72,15)
%     \begin{itemize}
%       \only<1->{\item Thread id (descriptor)}
%     \end{itemize}
%     \begin{itemize}
%       \only<2->{\item Interface functions operate on thread
%         descriptor}
%       \begin{itemize}
%         \only<2->{\item block/wake up thread...}
%       \end{itemize}
%     \end{itemize}
%     %\begin{itemize}
%      % \only<3>{\item Access the thread (resource)}
%     %\end{itemize}
%   \end{textblock}
% \end{frame}

\begin{frame}
  \frametitle<1-6>{Interface Description Model -- Descriptor and
    Resource}
  \begin{textblock}{110}(8,14)
    \only<1>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_lock_no_res}}%
    \only<2>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_lock_interface_funcs}}%    
    %\only<3>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_lock}}%
    \only<3-4>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_lock_SM}}% 
    \only<5-6>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/intro_descriptor_lock}}% 
  \end{textblock}
  \begin{textblock}{52}(72,15)
    \begin{itemize}
      \only<1-2, 5-6>{\item Lock id (lock descriptor)}
    \end{itemize}
    \begin{itemize}
      \only<2, 5-6>{\item Lock server provides interface functions}
      \begin{itemize}
        \only<2, 5-6>{\item init/free/take/release...}
      \end{itemize}
    \end{itemize}
    \begin{itemize}
      \only<2, 5-6>{\item Interface functions operate on lock descriptor}
    \end{itemize}
    \begin{itemize}
      \only<5-6>{\item Access the synchronization (lock resource)}
    \end{itemize}

  \PutAt<6>{(3.8cm,7.3cm)}{
    \NormalBox[text width=7.9cm]{
      \footnotesize{
        \begin{itemize}
        \item descriptor can be accessed by multiple components
        \item thread can block when access a resource
        \item ...
        \end{itemize}
      }
    }
  ...
  \begin{textblock}{120}(10,88)
    \only<4> {\Large {\it lock, scheduler, memory manager, file
        system, event...}}
  \end{textblock}
...
\begin{frame}
  \frametitle<1-5>{Component Interface Function Types}
  \begin{textblock}{110}(8,14)
    \only<1>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_3_0}}%
    \only<2>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_3_1}}%
    \only<3>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_3_2}}%
    \only<4-5>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_3}}%
  \end{textblock}
  
  \begin{textblock}{52}(72,15)
    \begin{itemize}
      \only<1->{\item create new descriptor}
      \only<2->{\item terminate descriptor}
      \only<3->{\item block invoking thread}
      \only<4->{\item wake up blocking thread}
    \end{itemize}
  \end{textblock}

  \begin{textblock}{120}(10,88)
    \only<5> {\Large {\it lock, scheduler, memory manager, file
        system, event...}}
  \end{textblock}
\end{frame}
...
\begin{frame}{\sg\ -- IDL-based System-Level Fault Tolerance}
  \begin{textblock}{110}(8,14)
    \only<1>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/overview_4}}%
  \end{textblock}
\end{frame}

\begin{frame}{Fault Recovery Model}
  \begin{textblock}{110}(8,14)
    \only<1>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/recovery_detection}}%
    \only<2>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/recovery_basic}}%
    \only<3>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/recovery_dependency}}%
    \only<4-5>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/recovery_all_in_one}}%
  \end{textblock}
  \begin{textblock}{80}(45,16)
    \only<2->{\item {\it \color{blue} Basic Recovery}}
    \begin{itemize}
      \only<2->{\item always through component operation}
    \end{itemize}
    \only<3->{{\it \color{blue} Recovery with Dependency}}
    \begin{itemize}
      \only<3->{
      \item recover the dependency descriptor first
      \item e.g., {\it new\_fd = accept(fd)}.  Descriptor {\it fd}
        needs to be recovered before {\it new\_fd}}
    \end{itemize}
    \only<4->{{\it \color{blue} Timing of Recovery}}
    \begin{itemize}
      \only<4->{\item on-demand: recover only when access}
      \only<4->{\item eager: wake up all blocking threads}
    \end{itemize}
    \only<4->{{\it {\color{blue} Recovery with Data Storage}}}\\
    \only<4->{{\it {\color{blue} Recovery with Upcall}}}
  \end{textblock}
  \begin{textblock}{120}(10,88)
    \only<5> {\Large {\it lock, scheduler, memory manager, file
        system, event...}}
...
\begin{frame}{System Model $\rightarrow$ Recovery Mechanisms}
  ...
  \PutAt<2>{(1.6cm,3.5cm)}{ \ColorBox[text width=9cm]{ \LARGE {Which
        recovery mechanisms to use?\\ How to generate the recovery
        code?\\}}}

  \PutAt<3>{(1.6cm,3.5cm)}{ \ColorBox[text width=9cm]{ \LARGE {Which
        recovery mechanisms to use?\\ How to generate the recovery code?\\
        \vspace{0.4cm} {\bf Idea:} an IDL compiler parses IDL
        specifications and generates the code}}}
\end{frame}

\begin{frame}{Example -- Client Invocation Stub}
  \vspace{-4cm}
  \begin{itemize}
  \item Predicate $\Rightarrow$ {\it ``always included''} 
...
\bigskip
  ...
  \item Transient faults modeled as random register bit-flips
    \begin{itemize}
    \item Software-implemented Fault Injection (SWIFI)
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Web Server Evaluation with Injected Faults}
    \includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/v1/WEB}
\end{frame}
...
\begin{frame}{Conclusion}
  \sg\ -- 
  \begin{itemize}
  \item automatically generates system-level fault recovery code
  %\item synthesizes recovery code from the high-level system model
  \item effectively eases the programmer burden
  \item provides efficient and predictable fault recovery
  \end{itemize}
\end{frame}
