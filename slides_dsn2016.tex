...
\makeatother
\author{Jiguo Song, Gedare Bloom, Gabriel Parmer \\Presented by {\bf Teodor Georgiev}}
  ...
  \begin{textblock}{52}(72,15)
    ...
    \begin{itemize}
      \only<2->{\item Interface functions operate on the file
        descriptor}
      \begin{itemize}
        \only<2->{\item open/close file, read/write file...}
      \end{itemize}
    \end{itemize}
    %\begin{itemize}
...
\begin{frame}
  ...
  \begin{textblock}{52}(72,15)
    \begin{itemize}
      \only<1-2, 5-6>{\item Lock id (descriptor)}
    \end{itemize}
    \begin{itemize}
    ...
    \end{itemize}
    \begin{itemize}
      \only<5-6>{\item Access the synchronization (resource)}
    \end{itemize}
  ...
  \begin{textblock}{80}(45,16)
    ...
    \begin{itemize}
      ...
      \item e.g., {\it new\_fd = accept(fd)}.  Descriptor {\it fd}
        must be recovered before {\it new\_fd}}
    \end{itemize}
...
\begin{frame}{Conclusion}
  \sg\ -- code generation for system-level fault tolerance 
  \begin{itemize}
