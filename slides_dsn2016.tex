  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      ...
      \item temporal redundancy
        ...
        \end{itemize}
      \item re-execution impacts only
...
\begin{frame}{\cos: A Component-Based OS}
  \begin{columns}
    \begin{column}{0.75\textwidth}
      ...
      {\em invocation} of exported function
      \begin{itemize}
      \item contractually specified interfaces
      \item function call semantics
      \end{itemize}

      \vspace{0.5em}
      Threads orthogonal to components
      \begin{itemize}
      \item thread migration -- predictable e2e IPC
      \item concurrent/parallel components
      \end{itemize}
    \end{column}
...
\begin{frame}{\sg\ -- IDL-based System-Level Fault Tolerance}
  ...
  \setbeamercolor{block
    body}{use=structure,fg=black,bg=grey!20!white}
  \begin{block}{\footnotesize Fault Model Assumption (same as \c3)}\scriptsize{  
      \begin{itemize}
      \item processor single-event upset and fault is detected
        immediately ({\sl fail-stop})
      \item two consecutive faults are separated by the minimum time
...
\begin{frame}
  \frametitle<1-8>{System Model -- Descriptor and Resource}
  \frametitle<9-15>{System Model -- Component Interface Functions}
  \begin{textblock}{110}(8,14)
    \only<1-4>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_1}}%
    \only<5-8>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_2}}%
    \only<9>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_3}}%
    \only<10>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_4}}%
    \only<11-15>{\includegraphics[width=1\linewidth]{SuperGlueFigs/design/dsn/design_1_3}}%
  \end{textblock}
  \begin{textblock}{52}(72,15)
    \only<1-8>{{\it Descriptor}}
    \begin{itemize}
      \only<2-8>{\item accessed locally or globally}
      \only<3-8>{\item meta data}
      \only<4-8>{\item dependent descriptor}
    \end{itemize}
    \only<5-8>{{\it Resource}}
    \begin{itemize}
      \only<6-8>{\item thread can block}
      \only<7-8>{\item meta data}
    \end{itemize}
    \only<9->{{\it Component Interface Functions}}
    \begin{itemize}
      \only<11->{\item create descriptor}
      \only<12->{\item terminate descriptor}
      \only<13->{\item block invoking thread}
      \only<14->{\item wake up thread}
    \end{itemize}
  \end{textblock}
  \begin{textblock}{120}(10,88)
    \only<8,15> {\Large {\it lock, scheduler, memory manager, file
        system, event...}}
  \end{textblock}
\end{frame}
...
\begin{frame}{Set of Recovery Mechanisms in \c3}
  \begin{textblock}{110}(8,14)
...
\begin{frame}{Example -- Client Invocation Stub}
  \vspace{-4cm}
  \begin{itemize}
  \item Predicate $\Rightarrow$ {\it ``true''} 
...
\begin{frame}{Example -- Client Stub Descriptor State Tracking}
  \vspace{-4cm}
  \begin{itemize}
  \item Predicate $\Rightarrow$ {\it ``the interface function can create
...
\section{Result and Experiments}

% \begin{frame} {\red{TO BE REMOVED - Result}}
% Show some results and experiments here
% \begin{itemize}
% \item LOC comparison (1 page)
% \item fault injection result (needs to add more here) (1 page)
...
\begin{frame}
  \frametitle{Fault Injection Experiments Setup}
  \begin{itemize}
  \item \sg\ implemented in {\em Composite} component-based OS
    \begin{itemize}
    \item Intel i7-2760QM, running at 2.4 Ghz
    \end{itemize}
  \item 6 services evaluated:
    \begin{itemize}
    \item the system scheduler (Sched)
    \item the system physical memory manager and mapper (MM)
    \item a RAM-based file system (FS)
    \item the lock manager (Lock)
    \item the system event manager (Event)
    \item the system timer manager (Timer)
    \end{itemize}
  \item Transient faults (processor single-event upsets)
    \begin{itemize}
    \item modeled as random register bit-flips (SWIFI)
    \end{itemize}
  \end{itemize}
\end{frame}
